...
lwhite="\e[1;37m"
bgtext="\e[48;5;17m"
bgtext2="\e[48;5;17m\e[1;37m"
THOME="/data/data/com.termux/files/home"
Verror="\e[48;5;19m\e[31mKESALAHAN\e[37m: "
SGTR="77de6634618b0833cdc56b309759b8c105f5ed7c5419be74e74afbf1eff15a9e"

reload-termux-set(){ am broadcast --user 0 -a com.termux.app.reload_style com.termux; }

this_termux(){
	if [ "$(uname -o)" == "Android" ]; then
		if su -c exit &> /dev/null; then
			if [ "$(whoami)" == "root" ]; then
				echo -e "${bgtext}\e[0;31mROOT TERDETEKSI\e[0m\n${bgtext}\e[0;37mTERMUX ANDA DALAM MODE ROOT, AGAR SKRIP INI BISA DIJALANKAN MOHON KELUAR DARI MODE ROOT.\e[0m"
				exit 1
			fi
		fi
	else
		echo -e "${bgtext}\e[0;33mSKRIP INI HANYA UNTUK TERMUX.\e[0m"
		exit 1
	fi
}
...
file-manager() {
	...
	if [ $typefile "$nfile" ]; then
		...
		if [ "$fromtc" == "y" ]; then
			...
			echo -e "${bgtext2}$nfile: TIDAK DAPAT MENEMUKAN FILE/FOLDER$dft"
			exit 1
		fi
	fi
	...
	if [ "$maker_0" == "root" ]; then
		echo -e "${bgtext} \e[1;34m${nfile}$dft${bgtext}\e[0;33mTERBUAT OLEH \e[0;31mROOT\e[0m ${bgtext}\e[0;33mSKRIP INI TIDAK BISA MENGAKSESNYA\e[0m"
		exit 1
	fi
	...
	if ((error > 0)); then
		echo -e "${bgtext2}SKRIP INI TIDAK BISA MENGAKSES $nfile KARENA TIDAK ADA IZIN \e[0;32m$permission$dft\n${bgtext2}BERIKAN IZIN AGAR SKRIP INI BISA MENGAKSESNYA.$dft"
		...
		while :; do
			...
			if [ "${userpermission,,}" == "y" ]; then
				if $givepermission 2> /dev/null; then
					echo -e "${bgtext}\e[0;32mBERHASIL\e[0m MEMBERI IZIN $permission PADA $nfile$dft"
					break
				else
					echo -e "${bgtext}\e[0;31mGAGAL\e[0mL MEMBERI IZIN $permission PADA $nfile$dft"
					break
				fi
...
TCsave()
{
	...
	if [ -z "$1" ] && [ -z "$2" ] && [ -z "$3" ]; then
		echo "${bgtext2}#TEMA DEFAULT$dft" > $filecolors
	else
		if ! echo -e "$1\n$2\n$3" > $filecolors 2> /dev/null; then
			echo -e "${Verror}GAGAL MENYIMPAN PEMBARUAN$dft"
			exit 1
		fi
	fi

	reload-termux-set > /dev/null
	return 0
}

TCHELP() {
	echo -en "${bgtext}${lwhite}"
echo -en "Untuk memilih warna, Pengguna bisa
memilihNya dengan angka atau nama
warna yang ada di menu skrip ini
contohNya: ${lgreen}LightGreen${lwhite} atau ${lgreen}lightgreen${lwhite}
...
REPORT TO \e[4;1;36mhttps://facebook.com/XklienZ$dft
${bgtext}TEKAN${lgreen} q ${lwhite}UNTUK KELUAR DARI SINI${dft}
"
	read -sd q
	line-clear 23
}
...
TCfullcolor() {
	...
	while :; do
		...
		echo -e "${bgtext}${lred}b).KEMBALI KE MENU AWAL"
		echo -e "${lwhite}?).BANTUAN"
		echo -e "${bgtext}${lgreen}PILIH WARNA UNTUK ${bgtext}\e[1;35m${1}"
		echo -e "$lyellow$back"
		echo -e "${lyellow}>).LEWATI"
		echo -e "${lwhite}$nextcolor"
		echo -e "${lwhite}$backcolor"
		echo -e " ${lgreen}0).Default"
		echo -e " ${colorlist[@]:${cds}:$pause}$dft"
		
		echo -en "$menu_notfound$dft\n${bgtext}${lcyan}X${TCprompt}${lcyan}Z${lwhite}${dft}> "
		read -r C_Usr
		...
		if [ -v fullcolor["${C_Usr}"] ]; then
			if [ "$1" == "${type[0]}" ]; then
				...
				if [ "$cfg" == "$cbg" ]; then
					menu_notfound="${bgtext}${yellow}WARNA SAMA DENGAN SEBELUMNYA$dft"
					line-clear $clr
					continue
				fi
			...
			menu_notfound="${bgtext}${red}TIDAK ADA MENU/WARNA$dft"
			line-clear $clr
		fi
	done
}
...
TFextrak() {
	echo -e "${bgtext2}Mengecek package unzip$dft"

	if command -v unzip > /dev/null; then
		echo -e "${bgtext2}Package unzip ditemukan$dft"
	else
		echo -e "${bgtext2}Package unzip tidak ditemukan$dft"
		echo -e "${bgtext2}Menginstall package unzip$dft"
		read < <(apt install unzip --assume-yes > /dev/null)
	
		if command -v unzip > /dev/null; then
			echo -e "${bgtext2}Package unzip berhasil di install$dft"
		else
			echo -e "${bgtext2}Package unzip gagal di install$dft"
			exit 1
		fi
	fi
	
	echo -e "${bgtext2}Mengekstrak file font-termux.zip$dft"
	unzip font-termux.zip > /dev/null
	
	if [ -f "$filefont" ]; then
		echo -e "${bgtext2}Berhasil di ekstrak$dft"
	else
		echo -e "${bgtext2}Gagal di ekstrak$dft"
		exit 1
	fi
...
TFdownloadfont() {
	local filename=font-termux.zip

	echo -e "${bgtext}${lcyan}MENGUNDUH FONT..$dft\n${bgtext}${lcyan}TUNGGU HINGGA PROSES PENGUNDUHAN SELESAI$dft"

	curl -L "https://drive.google.com/u/0/uc?id=1D_yD5UIU6UIq2I89-__W2wscgiF_acby&export=download" -o $filename

	if [ -f font-termux.zip ]; then
		local SGTR_U=$(sha256sum font-termux.zip | cut -d ' ' -f1)
		
		if [ "$SGTR" == "$SGTR_U" ]; then
			TFextrak
		else
			echo -e "${Verror}PADA FILE font-termux.zip$dft"
			exit 1
		fi
	else
		echo -e "${bgtext}${lwhite}File ${lgreen}font-termux.zip ${lwhite}tidak ditemukan didirektori saat ini$dft"
		exit 1
	fi
}
...
TFsave() {
	...
	if [ -z "$savefont" ]; then
		if [ -f "$termfont" ]; then
			if ! rm -f "$termfont"; then
				echo -e "${Verror}GAGAL MENGHAPAUS FILE font.ttf DI DIREKTORI $THOME/.termux$dft"
				exit 1
			else
				reload-termux-set > /dev/null
				return 0
	...
	while :; do
		...
		if ((fm_err==1)); then
			if [ -f font-termux.zip ]; then
				...
				echo -e "${Verror}TIDAK DAPAT MENEMUKAN ${lgreen}$filefont${dft}"
				
				echo -e "${bgtext}${lwhite}UKURAN FILE 17MB (sebelum  diekstrak)"
				echo -e "UKURAN ASLI 33MB (setelah diekstrak)"
				echo -en "UNDUH SEMUA FONT ? y/n:$dft "
				read -r downloadfont
	...
	if cp $filefont $THOME/.termux; then
		if mv $THOME/.termux/$savefont $termfont; then
			...
			echo -e "${Verror}GAGAL MENGGANTI NAMA FILE $savefont MENJADI $termfont$dft"
			exit 1
		fi
	else
		echo -e "${Verror}GAGAL MEMINDAHKAN FILE $savefont KE $termfont$dft"
		exit 1
	fi
	reload-termux-set > /dev/null
}
...
TFmenu() {
	local FCprompt menu_notfound clears F_Usr fontlist
	...
	while :; do
		echo -e "${bgtext}${lyellow}b).KEMBALI KE MENU AWAL"
		echo -e "${bgtext}${lgreen}PILIH FONT UNTUK TERMUX ANDA"
		echo -e "${lwhite}$nextfont"
		echo -e "$backfont"
		echo -e " ${lcyan}0).Default"
		echo -e " ${fontlist[@]:${menu_control}:9}${dft}"

		echo -en "${bgtext}${lwhite}$menu_notfound$dft\n${bgtext}${lblue}X$FCprompt${lblue}Z$dft> "
		read -r F_Usr
		...
		if [ -v fontconvert["$F_Usr"] ]; then
			...
			menu_notfound="${bgtext}${red}TIDAK ADA MENU/FONT$dft"
			line-clear $clears
		fi
	done
}
...
main-menu() {
	...
	while :; do
		echo -e "${bgtext}${lcyan}1).${lwhite}WARNA (BACKGROUND/FOREGROUND/CURSOR)"
		echo -e "${bgtext}${lcyan}2).${lwhite}FONT TERMUX$dft\n"

		echo -en "${bgtext}${lwhite}${menu_notfound}\n${bgtext}${lwhite}MENU-CBFC:>$dft "
		read -rsN 1 choosemenu

		case "$choosemenu" in
			1)
				line-clear $clrs; TCsettype;;
			2)
				line-clear $clrs; TFmenu;;
			*)
				menu_notfound="${bgtext}${red}TIDAK ADA MENU$dft"
				line-clear $clrs;;
		esac
	done
}
