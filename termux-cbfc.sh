...
#MASALAH GAGAL SAAT INGIN DIBERI IZIN
dft="\e[0m"
red="\e[31m"
lred="\e[1;31m"
yellow="\e[33m"
...
TFsave() {
	...
	while :; do
		...
		if ((fm_err==1)); then
			...
			echo -en "UNDUH SEMUA FONT ? y/n: "; read downloadfont
			if [ "${downloadfont,,}" == "y" ]; then
				echo -e "Mengunduh font.."
				#disini download dari googledrive
				file-manager font-termux "-d" "y" "-x"
				if ((fm_err=0)); then
					echo -e "Font berhasil di unduh di direktori ${lblue}$(pwd)$dft"
					file-manager "$filefont" "-f" "n"
				elif ((fm_err=1)); then
					file-manager font-termux "-d" "n" "-x"
					echo -e "Direktori ${lblue}font-termux$dft tidak ditemukan didirektori saat ini"
					exit 1
				fi
